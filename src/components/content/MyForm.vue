...
import { fetchDetail } from '@/services/bilibili-service';
 ...
 * 搜索按钮点击事件
 */
async function on_click_get_detail() {
    ...
    await set_video_id_and_video_detail(video_id);
 ...
 * @param {string} id
 */
async function set_video_id_and_video_detail(id) {
    try {
        show_loading_modal(true);
        const detail = await fetchDetail(id);
        video_id.value = id;
        video_detail.assign(detail);
    } catch (error) {
        video_id.value = '';
