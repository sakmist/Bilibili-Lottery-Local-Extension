...
import { checkIsMyFans } from '@/services/bilibili-service';
 ...
 */
async function on_click_is_my_fans(user) {
    try {
        show_loading_modal(true);
        const relation = await checkIsMyFans(user.id);
        user.relation_type = relation.relation_type;
        user.relation_type_description = relation.relation_type_description;
        user.relation_date = relation.relation_date;
    } catch (error) {
